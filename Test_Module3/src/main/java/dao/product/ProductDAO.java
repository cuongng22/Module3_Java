...
public class ProductDAO implements IProductDAO{
    ...
    public boolean updateById(int id, Product product) {
        try{
            PreparedStatement preparedStatement = connection.prepareStatement("UPDATE categories set name = ?, price = ?, quantity = ?, color = ?, description = ?, category_id = ? where id= ?");
            preparedStatement.setString(1, product.getName());
            preparedStatement.setDouble(2, product.getPrice());
            preparedStatement.setInt(3, product.getQuantity());
            preparedStatement.setString(4, product.getColor());
    ...
    public List<Product> findByName(String name) {
        ...
        Product product;
        try {
            PreparedStatement preparedStatement = connection.prepareStatement("SELECT * from product where name like %name%");
            ...
            while (rs.next()) {
                ...
                String description = rs.getString("description");
                int category_id = rs.getInt("category_id");
                product = new Product(p_id, p_name,price, quantity, color, description, category_id );
                products.add(product);
            }
        } catch (SQLException e) {
            e.printStackTrace();
